...
chromium::base的EventSingleThread强依赖于chromium::base，跟剥离chromium::base的初衷相悖
EventSingleThread reference to: 
...
namespace gob {
	...
	class EventSingleThread {
		// 一个线程处理所有的事件
	public:
		EventSingleThread(const std::string& thread_name);
		~EventSingleThread();
		bool start();
	...
	private:
		// 禁止拷贝
		EventSingleThread(const EventSingleThread&) = delete;
		EventSingleThread& operator=(const EventSingleThread&) = delete;
		void stop();
